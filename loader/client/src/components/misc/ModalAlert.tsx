...
export const ModalAlert:React.FC<{children:React.ReactNode }> = ({children}):React.ReactNode => {
 const [ hide, setHide ] = useState(false)
  return (
    <>
<div id="confirmation" className={hide ? "hidden":`fixed z-10 inset-0 overflow-y-auto`}>
        <div  className="flex items-center justify-center h-screen pt-4 px-4 pb-20 text-center">
            {/*<div className="fixed inset-0 transition-opacity">
                {<div className="absolute inset-0 bg-gray-500 opacity-75"></div>}
            </div>*/}
           
            <span onClick={()=>setHide(false)} className="sm:inline-block sm:align-middle sm:h-screen"></span>&#8203;
            
            <div className="inline-block align-bottom bg-gray-500 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
